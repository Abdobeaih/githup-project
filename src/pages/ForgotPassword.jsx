...
export default function ForgotPassword() {
  ...
  const [form, setForm] = useState({ email: '', role: USER_ROLES.USER, password: '', confirmPassword: '' })
  ...
  const handleSendReset = (e) => {
    ...
    setError('')
    setSuccess('')
    const result = forgotPassword(form.email, form.role)
    if (result.success) {
      setStep('reset')
      setSuccess(result.message || t('forgotPassword', 'success'))
    } else {
      setError(result.error || t('forgotPassword', 'errorEmailNotFound'))
    }
  }
  ...
  const handleResetPassword = (e) => {
    ...
    const result = resetPassword(form.email, form.password, form.role)
    if (result.success) {
      setStep('done')
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="max-w-md mx-auto">
            ...
            <div className="bg-white/5 backdrop-blur-xl p-8 rounded-3xl border border-gold/20">
              <AnimatePresence mode="wait">
                ...
                {step === 'email' && (
                  <motion.form key="email" onSubmit={handleSendReset} className="space-y-5"
                ...
                {step === 'reset' && (
                  <motion.form key="reset" onSubmit={handleResetPassword} className="space-y-5"
                    ...
                    <div className="text-center mb-4">
                      <p className="text-goldLight/80 text-sm">{success}</p>
                    </div>
